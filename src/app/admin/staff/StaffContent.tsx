...
export default function StaffContent() {
  ...
  const ROLE_LABELS: Record<string, string> = {
    owner: t.admin.staffPage.roles.owner,
    admin: t.admin.staffPage.roles.admin,
    staff: t.admin.staffPage.roles.staff,
    client: "Cliente",
  };
      ...
      <div className="bg-white/[0.02] border border-white/5 rounded-2xl p-5 space-y-3 mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
          <input
            ...
            placeholder={t.admin.staffPage.name}
          ...
          <input
            ...
            placeholder={t.admin.staffPage.email}
            className="w-full px-3 py-2 bg-white/5 border border-white/10 rounded-xl text-white placeholder-white/30 text-sm focus:outline-none focus:ring-2 focus:ring-amber-500/50"
          />
          <select
            value={newRole}
          ...
          >
            <option value="staff">{t.admin.staffPage.roles.staff}</option>
            <option value="admin">{t.admin.staffPage.roles.admin}</option>
          </select>
        </div>
        <button
          onClick={addMember}
